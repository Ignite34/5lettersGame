...
public PlayerController playerController;
  ...
  if (currentRow > amountOfRows)
 {
  ...
  if (wordBoxes[(currentRow * charactersPerRowCount) + currentWordBox].GetChild(0).GetComponent<TextMeshProUGUI>().text == "")
  {
    wordBoxes[(currentRow * charactersPerRowCount) + currentWordBox].GetChild(0).GetComponent<TextMeshProUGUI>().text = letter;
...
IEnumerator CheckWord(string guess)
{
  // Set up variables
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        TextMeshProUGUI currentWordboxColor = wordBoxes[i + (currentRow * charactersPerRowCount)].GetChild(0).GetComponent<TextMeshProUGUI>();
...
if (currentRow > amountOfRows)
    {
        ShowPopup("Повезёт в следующий раз :(\n" + "Правильное слово: " + correctWord, 0f, true);
        yield return null;
    }
